...
class AutopilotPage extends StatefulWidget {
  ...
  final int hrThresholdMin; // 心拍数の下限
  final int hrThresholdMax; // 心拍数の上限

  const AutopilotPage({
    super.key,
    required this.heartRateStream,
    required this.initialPwmValue,
    required this.hrThresholdMin,
    required this.hrThresholdMax,
  });
      ...
      if (bpm != null && bpm.isFinite) {
        setState(() {
          ...
          currentPwm = AutopilotController.pwmFromHeartRate(
              bpm,
              widget.hrThresholdMin.toDouble(),
              widget.hrThresholdMax.toDouble());
